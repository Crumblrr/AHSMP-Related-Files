-- THIS SCRIPT WAS MODIFIED BY CRUMB'S DAD. IF YOU'RE SEEING THIS AND YOU'RE FROM ARTHOUSE, THIS IS THE CORRECT VERSION OF THE SCRIPT. DON'T TOUCH IT. 
-- IF YOU HAVE ISSUES WHEN USING THIS SCRIPT, PLEASE DO NOT BOTHER NOR BLAME THE ORIGINAL AUTHORS. CONTACT CRUMB WITH A SCREENSHOT OF THE ERROR YOU'RE ENCOUNTERING.
...
local physBoneBase = {
	...
	setLength =
		function(self,data)
			self.length = data
			self.worldlength = data / 16 / math.worldScale
...
events.entity_init:register(function()
	...
	for _,boneID in pairs(physBoneIndex) do
		physBone[boneID]:setPathChildren(physBone[boneID].path:getChildren())
		physBone[boneID]:setLength(physBone[boneID]:getLength()) -- Sorry, nasty hack to ensure worldlength exists for all bones.
	end
	...
	for colID,collider in pairs(physBone.collider) do
		...
		if faces.s then 
			colGroup.s = {size = vec(size.x,size.y,size.z) / worldScale}
		end
		if faces.n then 
			colGroup.n = {size = vec(size.x,size.y,size.z) / worldScale}
		end
		if faces.e then 
			colGroup.e = {size = vec(size.z,size.y,size.x) / worldScale}
		end
		if faces.w then 
			colGroup.w = {size = vec(size.z,size.y,size.x) / worldScale}
		end
		if faces.u then 
			colGroup.u = {size = vec(size.x,size.z,size.y) / worldScale}
		end
		if faces.d then 
			colGroup.d = {size = vec(size.x,size.z,size.y) / worldScale}
		end
	end
...
events.RENDER:register(function (delta,context)
	...
	for colID,collider in pairs(physBone.collider) do
		...
		if faces.s then
			...
			colGroup.s.pos = colPos
			colGroup.s.normalX = colNormalX
			colGroup.s.normalY = colNormalY
			colGroup.s.normalZ = colNormalZ
		end
		if faces.n then
			local colPos = (colTransMat * offsetMat:copy():translate(-size)):translate(partPos):apply()
			colGroup.n.pos = colPos
			colGroup.n.normalX = -colNormalX
			colGroup.n.normalY = -colNormalY
			colGroup.n.normalZ = -colNormalZ
		end
		if faces.e then
			local colPos = (colTransMat * offsetMat:copy():translate(-size * vec(0,1,0))):translate(partPos):apply()
			colGroup.e.pos = colPos
			colGroup.e.normalX = colNormalZ
			colGroup.e.normalY = -colNormalY
			colGroup.e.normalZ = colNormalX
		end
		if faces.w then
			local colPos = (colTransMat * offsetMat:copy():translate(-size * vec(1,0,1))):translate(partPos):apply()
			colGroup.w.pos = colPos
			colGroup.w.normalX = -colNormalZ
			colGroup.w.normalY = colNormalY
			colGroup.w.normalZ = -colNormalX
		end
		if faces.u then
			local colPos = (colTransMat * offsetMat:copy():translate(-size * vec(0,0,1))):translate(partPos):apply()
			colGroup.u.pos = colPos
			colGroup.u.normalX = colNormalX
			colGroup.u.normalY = -colNormalZ
			colGroup.u.normalZ = colNormalY
		end
		if faces.d then
			local colPos = (colTransMat * offsetMat:copy():translate(-size * vec(1,1,0))):translate(partPos):apply()
			colGroup.d.pos = colPos
			colGroup.d.normalX = -colNormalX
			colGroup.d.normalY = colNormalZ
			colGroup.d.normalZ = -colNormalY
		end
	end

	if (recreatewhitelists) then
		recreatewhitelists = false
		for _,boneID in pairs(physBoneIndex) do
			physBone[boneID].whitelist = {}
			local ind = 0
			for groupID,group in pairs(colliderGroups) do
				if not physBone[boneID].colliderBlacklist[groupID] then
					for _,face in pairs(group) do
						physBone[boneID].whitelist[ind] = face
						ind = ind + 1
					end
				end
			end
...
function physBone.physBoneRender(delta, context, curPhysBoneID)
	...
	if not hasCollided then
		local nextPos = pendulumBase + direction * (curPhysBone.worldlength)
		...
		local nextPos = pendulumBase + colNextPos:normalized() * (curPhysBone.worldlength)
	...
	-- Transform matrix
		...
		if curPhysBone.path:getVisible() then
			...
			local bonemat = matrices.mat4()

			bonemat:translate(-parentPivot)
			bonemat:rotate(0,curPhysBone.rotMod.y,curPhysBone.rotMod.z)
								:rotate(curPhysBone.rotMod.x,0,0)
								:rotate(0,curPhysBone.rollMod-90,0)
			bonemat:rotate(pitch,0,yaw)
							:translate(parentPivot)

			for _,part in pairs(curPhysBone.pathChildren) do
				if part:getVisible() then
