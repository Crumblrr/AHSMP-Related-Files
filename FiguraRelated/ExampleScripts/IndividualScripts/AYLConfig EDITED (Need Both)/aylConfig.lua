...
-- MODIFIED BY: CRUMB (@crumble0002 on Discord)
...
function aylConfig:init(dPages, dConfigs)
    ...
    for i,v in pairs(configs) do
        ...
        elseif configs[i].type == "button" then     -- Button added by Crumb
            configs[i].action:setOnLeftClick(pings[i])
            configs[i].action:setOnRightClick(pings[i])
...
-- Save funcs
-- Saving Button Action (Added by Crumb)
function aylConfig:saveButton(key, value)
